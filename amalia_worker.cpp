...
static void run_worker(Secp256K1 &secp, const char *master_host, int master_port,
    ...
    if(use_gpu){
        ...
        if(available_gpus==0) die("--gpu requested but no usable CUDA GPU found");
        int actual_gpus = num_gpus>0 ? num_gpus : 1;
        if(actual_gpus > available_gpus) actual_gpus = available_gpus;
        printf("[worker] GPU mode: using %d of %d available GPU(s)\n", actual_gpus, available_gpus);
        ...
        for(int g=0; g<actual_gpus; ++g){
            gpu_workers.emplace_back([&, g](){
                if(!gpu_set_device(g)){
                    fprintf(stderr, "[worker] GPU %d: failed to select device, this worker thread exiting\n", g);
                    return;
                }
                ...
                if(!hgpu_g){
                    fprintf(stderr, "[worker] GPU %d: failed to upload hierarchy, this worker thread exiting\n", g);
                    return;
                }
                printf("[worker] GPU %d: ready, connecting to %s:%d\n", g, master_host, master_port);

                while(true){
                    int cfd = net_connect(master_host, master_port);
                    if(cfd<0){ printf("[worker] GPU %d: could not connect to master, retrying in 5s\n", g);
                    ...
                    if(tok[0]=="STOP"){ printf("[worker] GPU %d: master signalled STOP - exiting.\n", g);
                        gpu_hierarchy_free(hgpu_g); return; }
                    if(tok[0]=="WAIT"){
                        double secs = tok.size()>=2 ? atof(tok[1].c_str()) : 2.0;
                        std::this_thread::sleep_for(std::chrono::duration<double>(secs)); continue;
                    }
                    if(tok[0]!="WORK" || tok.size()<6){ printf("[worker] GPU %d: unexpected response: %s\n", g, line.c_str());
                    ...
                    printf("[worker] GPU %d work #%" PRIu64 ": offset=0x%s block_size=%" PRIu64 "\n",
                           g, wid, offset.GetBase16(), block_size);
                    ...
                    bool hit = process_one_block_race_gpu(secp, st, hgpu_g, ctx, tree_steps, a_bits, threads,
                                                            ...
                                                            found_K, &leaf_count);
                    printf("[worker] GPU %d work #%" PRIu64 " done in %.2fs, %" PRIu64 " leaves, %s\n",
                           g, wid, now_seconds()-t0, leaf_count, hit?"FOUND":"not found");

                    int rfd = net_connect(master_host, master_port);
                    if(rfd<0){ printf("[worker] GPU %d: could not report result (master unreachable)\n", g); continue; }
                    char buf[256];
                    if(hit){
                        snprintf(buf,sizeof(buf),"RESULT %" PRIu64 " FOUND %s", wid, found_K.c_str());
                        printf("[worker] GPU %d *** FOUND *** (result withheld from worker log - see master) reporting to master.\n", g);
                    } else {
                    ...
                    close(rfd);
                    if(hit){ printf("[worker] GPU %d: done - answer found, exiting.\n", g);
                        gpu_hierarchy_free(hgpu_g); return; }
                }
            });
        }
        for(auto &w : gpu_workers) w.join();
        return;
    }
#else
    if(use_gpu) die("--gpu requested but this binary was built without GPU support - rebuild with 'make GPU=1'");
#endif
...
static void worker_usage(const char *prog){
    fprintf(stderr,
      "amalia_worker - standalone distributed-search worker (BSGS only)\n"
      "(companion to amalia_hier_leaf's --master; see that project's\n"
      "README.md for the full protocol and context)\n\n"
      "Usage: %s --master-host HOST --master-port PORT \\\n"
      "          --start-pubkey HEX --tree-steps N --root-bits N \\\n"
      "          --hier-table FILE \\\n"
      "          [--restrict-upper-half] [-t THREADS] [--gpu] [--gpus N]\n\n"
      "  --master-host HOST      master's address (default 127.0.0.1)\n"
      "  --master-port PORT      master's port (default 23941)\n"
      "  --start-pubkey HEX      target public key (66 hex chars, 02/03 prefix)\n"
      "  --tree-steps N          must match the --master this connects to\n"
      "  --root-bits N           must match the --master this connects to\n"
      "  --hier-table FILE       local path for the BSGS hierarchy - downloaded\n"
      "                          from master automatically if missing\n"
      "  --restrict-upper-half   must match the --master this connects to\n"
      "  -t THREADS              worker thread count (default: all detected) -\n"
      "                          only used in CPU mode; --gpu mode uses this\n"
      "                          machine's GPU(s) instead\n"
      "  --gpu                   use GPU acceleration (requires a GPU=1 build) -\n"
      "                          uses 1 GPU unless --gpus also given\n"
      "  --gpus N                use N GPUs (implies --gpu) - each GPU runs its\n"
      "                          own independent WORK/RESULT cycle against\n"
      "                          --master, as if N separate workers\n",
      prog);
}

int main(int argc, char **argv){
    const char *master_host = "127.0.0.1";
    int master_port = 23941;
    char *start_pubkey_hex = nullptr;
    int tree_steps = -1, root_bits = 64;
    const char *hier_table_file = nullptr;
    ...
    for(int i=1;i<argc;++i){
        if(!strcmp(argv[i],"--help")||!strcmp(argv[i],"-h")){ worker_usage(argv[0]); return 0; }
        else if(!strcmp(argv[i],"--master-host")){ master_host = next(i); }
        else if(!strcmp(argv[i],"--master-port")){ master_port = atoi(next(i)); }
        else if(!strcmp(argv[i],"--start-pubkey")){ start_pubkey_hex = next(i); }
        else if(!strcmp(argv[i],"--tree-steps")){ tree_steps = atoi(next(i)); }
        else if(!strcmp(argv[i],"--root-bits")){ root_bits = atoi(next(i)); }
        else if(!strcmp(argv[i],"--hier-table")){ hier_table_file = next(i); }
        else if(!strcmp(argv[i],"--restrict-upper-half")){ restrict_upper_half = 1; }
        else if(!strcmp(argv[i],"-t")){ threads = atoi(next(i)); }
        else if(!strcmp(argv[i],"--gpu")){ use_gpu = 1; }
        else if(!strcmp(argv[i],"--gpus")){ num_gpus = atoi(next(i)); use_gpu = 1; }
        else { fprintf(stderr, "ERROR: unknown argument '%s'\n", argv[i]); worker_usage(argv[0]); return 1; }
    }
    if(!start_pubkey_hex || tree_steps<0 || !hier_table_file){
        worker_usage(argv[0]);
        return 1;
    }
    if(threads<=0){
        ...
        printf("[worker] -t not set, auto-detected %d hardware threads\n", threads);
    }

    Secp256K1 secp; secp.Init();
    run_worker(secp, master_host, master_port, start_pubkey_hex, tree_steps, root_bits,
               hier_table_file, restrict_upper_half!=0, threads, use_gpu!=0, num_gpus);
    return 0;
}
